...
- It collects data from proxy traffic and JavaScript files.
- Results are saved to JSON files and shown in a handy UI tab.

**Key Features**

- Collect parameters and endpoints from requests/responses in Proxy and Repeater.
- Find endpoints in JavaScript by regex.
- Optional AST scan of JS with `jsluice`.
- Build a command to run `httpx` and “spray” selected endpoints across site tree paths.
...
- Optional tools:
  - `jsluice` (AST scan of JS)
  - `httpx` (endpoint spraying)
...
- Build with Gradle:
  - `./gradlew clean build`
  - `./gradlew jar` (creates the JAR with dependencies)
- Load in Burp:
  - Burp → Extensions → Installed → Add → choose the JAR from `build/libs/`
- Quick reload tip: Ctrl/⌘ + click the “Loaded” checkbox in Burp
